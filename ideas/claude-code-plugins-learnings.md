# Claude Code Plugins 學習筆記

## 來源

- **日期**：2025-12-12
- **參考**：https://github.com/anthropics/claude-code/tree/main/plugins
- **分析的 Plugins**：code-review, feature-dev, plugin-dev, security-guidance

## 探索狀態總覽

| 方向 | 狀態 | 說明 |
|------|------|------|
| 漸進式輸出 | ✅ 已升級 | `/atlas.pattern` 完成，其他擱置 |
| 持久化 | ✅ 已升級 | `--save` + `/atlas.clear` 完成 |
| 信心 Threshold | ⏸️ 擱置 | 無用戶需求，風險過濾有價值假設 |
| 並行 Agent | ⏸️ 擱置 | 增加複雜度，token 成本不明 |
| **輸出格式統一** | 🔍 探索中 | 方案 A（輕量統一）可行，~1.5h 工作量 |

---

## 值得探索的方向

### 1. 並行 Agent 架構

**來自**：code-review

**設計**：
- 4 個 Agent 並行執行（2 Compliance + 1 Bug + 1 History）
- 每個 Agent 有獨立 focus area
- 結果匯總後統一過濾

**潛在應用**：
- `/atlas.overview` 可並行分析：技術棧、架構、測試、文檔
- `/atlas.impact` 可並行追蹤：imports、exports、tests、configs

**待驗證**：
- [ ] Claude Code 的 Task tool 並行效能如何？
- [ ] 是否會增加 token 消耗（多個 agent 各自讀檔）？
- [ ] 結果合併的複雜度？

**SourceAtlas 現況**：單一執行，較慢但 token 可控

---

### 2. 漸進式輸出（Progressive Disclosure）✅ 已升級

**狀態**：已升級到 [proposals/progressive-disclosure/](../proposals/progressive-disclosure/)

**實作進度**：
- ✅ `/atlas.pattern` - Smart 模式 + `--brief`/`--full` (v2.9.3)
- ⚪ 其他命令 - 擱置（已有快取機制，效益有限）

**擱置原因**（2025-12-18）：
1. `/atlas.overview` 等命令通常只執行一次，已有 `--save` 快取
2. 增加維護複雜度，ROI 不高
3. 假設未經驗證，無使用者抱怨 token 成本

**重新評估條件**：使用者反饋「輸出太多」或 token 成本問題

---

### 3. 信心分數 + Threshold 機制 ⏸️ 擱置

**狀態**：擱置（2025-12-19）

**原始設計**：設定門檻（如 0.7），只輸出高於門檻的假設

**擱置原因**：
1. 無用戶反饋「假設太多/雜訊太多」
2. 風險：可能過濾掉有價值的低信心假設
3. 已有替代：`--brief` 參數可控制輸出量

**重新評估條件**：用戶抱怨「輸出太雜」

**SourceAtlas 現況**：有 0.0-1.0 信心分數，全部輸出由用戶判斷

---

### 4. Validation Subagent

**來自**：code-review

**設計**：
```
Step 4: 4 agents 並行找問題
Step 5: 對每個問題，啟動 validation subagent 驗證
Step 6: 過濾掉未通過驗證的問題
```

**潛在應用**：
- `/atlas.overview` Stage 0 生成假設後，用 subagent 驗證
- 取代目前的 Stage 1 手動驗證？

**待驗證**：
- [ ] 驗證 subagent 的 token 成本？
- [ ] 是否能達到 Stage 1 的驗證品質？
- [ ] 自動驗證 vs 人工驗證的取捨？

**SourceAtlas 現況**：Stage 1 是獨立步驟，需要人工觸發

---

### 5. Hook 機制轉 Constitution 規則

**來自**：security-guidance

**設計**：
```
PreToolUse hook → 在工具執行前驗證
可以阻擋不符合規則的操作
```

**潛在應用**：
```bash
# 把 Constitution Article II（排除目錄）轉為 hook
if [[ "$file_path" == *"node_modules"* ]]; then
  echo "BLOCKED: Constitution Article II violation"
  exit 1
fi
```

**待驗證**：
- [ ] SourceAtlas 是 slash command，不是 plugin，能用 hook 嗎？
- [ ] 是否需要把 SourceAtlas 轉為 plugin 架構？
- [ ] 目前 `validate-constitution.sh` 是否足夠？

**SourceAtlas 現況**：Constitution 是文檔 + 事後驗證，不是事前阻擋

---

### 6. 標準化輸出格式 🔍 深度探索中

**來自**：code-review

**設計**：
```markdown
## Code review

Found 3 issues:

1. [description] (CLAUDE.md says: "[exact quote]")
   https://github.com/.../file.ts#L67-L72
```

---

#### 🔍 深度分析（2025-12-19）

##### 現況盤點

| 命令 | 輸出格式 | Header | Footer | 證據格式 | Handoffs |
|------|---------|--------|--------|---------|----------|
| `/atlas.overview` | YAML | ❌ 無 | ❌ 無 | `file:line` | ✅ 有 |
| `/atlas.pattern` | Markdown | `# Pattern: [Name]` | `---` | `[File Path]:[line]` | ✅ 有 |
| `/atlas.history` | Markdown | `=== Smart Temporal Analysis ===` | `---` | `commit hash, file:line` | ✅ 有 |
| `/atlas.impact` | Markdown | `=== API/Model Impact Analysis ===` | `---` | `[path:line]` | ✅ 有 |
| `/atlas.flow` | Markdown | 無標準 | ✅ Analysis Metadata | `📍 file:line` | ✅ 有 |
| `/atlas.deps` | YAML | ❌ 無 | `## Next Steps` | `[path:line]` | ✅ 有 |

##### 發現的不一致

1. **Header 格式不統一**
   - `=== Title ===`（history, impact）
   - `# Pattern: Name`（pattern）
   - 無 Header（overview, deps）

2. **Evidence 格式不統一**
   - `file:line`
   - `[path:line]`
   - `📍 file:line`
   - `[File Path]:[line]`

3. **Handoffs 格式已統一** ✅
   - 全部使用 `## Recommended Next` + 表格

4. **混合 YAML/Markdown**
   - overview, deps → YAML
   - pattern, history, impact, flow → Markdown

##### 三種統一方案

| 方案 | 描述 | 優點 | 缺點 |
|------|------|------|------|
| **A. 輕量統一** | 只統一 Header/Footer/Evidence | 改動小 | 仍有差異 |
| **B. 格式統一** | 全部改為 Markdown | 一致性高 | overview/deps 破壞性變更 |
| **C. Schema 定義** | 定義標準 + 驗證腳本 | 可擴展 | 增加複雜度 |

**建議**：**方案 A（輕量統一）** - 投入產出比最高

##### 方案 A 具體設計（修訂版 2025-12-19）

經使用者訪談模擬後的最終設計：

**1. 統一 Header**
```markdown
🗺️ SourceAtlas: [Command Name]
───────────────────────────────
[emoji] [target] │ [key metric]
```

**2. 命令專屬 Emoji**
| 命令 | Emoji | 範例 Header |
|------|-------|-------------|
| overview | 🔭 | `🔭 my-app │ MEDIUM (2.5K files)` |
| pattern | 🧩 | `🧩 repository │ 15 files` |
| history | 📜 | `📜 src/ │ 12 months` |
| impact | 💥 | `💥 User model │ 23 dependents` |
| flow | 🔀 | `🔀 checkout │ 5 steps` |
| deps | 📦 | `📦 iOS 16→17 │ 35 APIs` |

**3. 統一 Footer**
```markdown
───────────────────────────────
🗺️ v2.9.4 │ Constitution v1.1
```

**4. Evidence 格式**
```
file:line          # 標準格式（保持不變）
```

**設計決策**：
- 分隔線 30 字元（避免窄 terminal 換行）
- 用 `│` 分隔資訊（比 `|` 更美觀）
- Footer 簡潔（版本 + Constitution）
- 不加 timestamp（按需再加）

##### 實作成本評估

| 命令 | 改動 | 工作量 |
|------|------|--------|
| `/atlas.overview` | 加 Header | 15min |
| `/atlas.pattern` | 改 Header 格式 | 10min |
| `/atlas.history` | 改 Header 格式 | 10min |
| `/atlas.impact` | 改 Header 格式 | 10min |
| `/atlas.flow` | 加統一 Header | 15min |
| `/atlas.deps` | 加 Header | 15min |

**總工作量**：約 1-1.5 小時

##### 風險評估

| 風險 | 機率 | 影響 | 緩解 |
|------|------|------|------|
| 用戶習慣改變 | 低 | 低 | Header 變化不大 |
| 破壞現有快取 | 無 | 無 | 只改輸出格式 |
| prompt 檔案過大 | 低 | 低 | 增加 <50 行 |

##### 結論

**可行性**：✅ 高
**價值**：
- 視覺一致性提升
- 品牌識別度（SourceAtlas 標準格式）
- 方便後續工具整合（解析器）

**建議**：可升級到 `proposals/`，但優先級中等

**SourceAtlas 現況**：各命令輸出格式有差異，但 Handoffs 已統一

---

## SourceAtlas 已經做得更好的地方

這些是 SourceAtlas 的優勢，不需要改：

1. **Constitution 治理** - Claude Code Plugins 沒有統一規則框架
2. **規模感知** - Plugins 是一刀切，SourceAtlas 有 5 級策略
3. **資訊理論** - 高熵優先、掃描比例上限
4. **141 Patterns** - 預定義 pattern，不依賴即時判斷
5. **Handoffs 原則** - 動態下一步，不是固定流程

---

## 優先級評估

| 方向 | 價值 | 難度 | 建議優先級 |
|------|------|------|------------|
| 漸進式輸出 | 高（token 效率） | 中 | ⭐⭐⭐⭐ |
| 信心 Threshold | 中（減少雜訊） | 低 | ⭐⭐⭐ |
| 並行 Agent | 中（速度） | 高 | ⭐⭐ |
| Validation Subagent | 低（已有 Stage 1） | 高 | ⭐ |
| Hook 機制 | 低（已有驗證腳本） | 高 | ⭐ |
| 輸出格式統一 | 中（一致性） | 中 | ⭐⭐⭐ |

---

## 下一步

- [x] ~~評估漸進式輸出的具體設計~~ → 已升級並部分完成
- [x] ~~測試信心分數 threshold~~ → 擱置（無用戶需求）
- [x] **審視各命令輸出格式，評估統一成本** → 完成探索（2025-12-19）
  - 發現：Header/Evidence 格式不統一，但 Handoffs 已統一
  - 建議：方案 A（輕量統一），約 1.5h 工作量
  - 決策：待定（是否升級到 proposals/）

## 升級條件

當滿足以下條件時，可將具體項目移至 `proposals/`：

1. 確定要實作哪個功能
2. 有具體的技術方案
3. 完成初步可行性驗證
4. 定義清晰的使用場景
