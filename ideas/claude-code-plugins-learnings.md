...
| 信心 Threshold | ⏸️ 擱置 | 無用戶需求，風險過濾有價值假設 |
| 並行 Agent | ⏸️ 擱置 | 增加複雜度，token 成本不明 |
| 輸出格式統一 | 🔲 待探索 | 有價值，提升一致性 |
...
### 3. 信心分數 + Threshold 機制 ⏸️ 擱置

**狀態**：擱置（2025-12-19）

**原始設計**：設定門檻（如 0.7），只輸出高於門檻的假設

**擱置原因**：
1. 無用戶反饋「假設太多/雜訊太多」
2. 風險：可能過濾掉有價值的低信心假設
3. 已有替代：`--brief` 參數可控制輸出量

**重新評估條件**：用戶抱怨「輸出太雜」

**SourceAtlas 現況**：有 0.0-1.0 信心分數，全部輸出由用戶判斷
...
- [x] ~~測試信心分數 threshold~~ → 擱置（無用戶需求）
- [ ] **審視各命令輸出格式，評估統一成本** ← 下一個探索方向
