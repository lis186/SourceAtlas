...
───────────────────────────────────
🎯 Recommended Next
├── Primary: /atlas.impact "[流程中的關鍵節點]"
│   └── Why: 發現此節點被 N 處調用，變更風險較高
└── Secondary: /atlas.pattern "[發現的 pattern]"
    └── Why: 流程中使用了 [pattern]，可學習實作慣例
───────────────────────────────────
```

**何時建議**（根據發現選擇最相關的）：
- 發現高耦合節點 → `/atlas.impact "[節點名稱]"`
- 流程涉及複雜 pattern → `/atlas.pattern "[pattern]"`
- 流程變動頻繁（從邊界標記推斷）→ `/atlas.history [相關檔案]`
- 發現其他相關流程 → `/atlas.flow "[相關流程]"`
