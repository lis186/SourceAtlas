...
- **v2.9.0** 🔵 - Dependency Analysis `/atlas.deps`（開發中）
...
- `/atlas.deps` 🔵 - Dependency 使用分析（開發中）⭐ NEW
...
**版本歷程**：
- v2.7.0 (2025-12-01): **Flow 分析完成** - `/atlas.flow` 命令，11 種分析模式，語言專屬入口點偵測，增強邊界識別 ⭐ NEW
- v2.6.0 (2025-11-30): **時序分析完成** - `/atlas.history` 命令，整合 code-maat，支援 Hotspots/Coupling/Contributors 分析
...
### 🔵 開發中 - v2.9.0 Dependency Analysis
- [ ] `/atlas.deps` - Dependency 使用分析（開發中）⭐ NEW
  - Library 使用點盤點
  - API 使用統計
  - Breaking Change 對照
