...
1. **假設未經驗證**：沒有證據顯示「history 需要 7 天更新」，很多專案 commit 頻率低，7 天警告會造成「狼來了」效應
2. **增加認知負擔**：使用者會困惑「為什麼 pattern 是 60 天，history 是 7 天？」，需要額外文檔解釋
3. **與 SourceAtlas 哲學衝突**：SourceAtlas 強調「簡單、直覺、標準優於自訂」，差異化閾值違反統一介面原則
4. **實作複雜度高**：需改 6 個檔案，未來新增命令容易遺漏同步

**替代方案**：保持 30 天統一閾值，讓使用者用 `--force` 自己判斷

**重新評估條件**：除非有明確證據證明不同閾值能解決真實問題，否則維持擱置
