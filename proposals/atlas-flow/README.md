...
3. DiscountEngine.apply()          → 計算折扣
   ...
   ├── CouponService.apply()       → 優惠券      🔍 [A]
   └── PointsService.redeem()      → 點數折抵    🔍 [B]

4. InventoryService.reserve()      → 預扣庫存
   📍 src/services/inventory.ts:156

5. PaymentService.process()        → 處理付款    🔍 [C]
...
🔍 可展開：
  [A] CouponService     - 優惠券邏輯
  [B] PointsService     - 點數折抵
  [C] PaymentService    - 付款流程

💬 輸入 A/B/C 展開，或「全部」展開所有
──────────────────────────────────
```
