...
| 版本類型 | 當前版本 | 說明 |
|---------|---------|------|
| **文檔版本** | v3.0 | 本提案文檔的修訂版本 |
| **目標產品版本** | v2.6 | 本提案計劃實作到 SourceAtlas v2.6 |

- **文檔版本** 追蹤本提案的設計變更（v1.0 → v2.0 → v2.1 → v3.0）
- **產品版本** 指 SourceAtlas 產品本身的版本（當前 v2.6.0，規劃中）

---

## 📋 更新歷史

### v3.0 (2025-11-30) - 單一命令設計 + 重新命名 ⭐⭐

#### 重大變更

**命令設計革新**：2 個命令 → **1 個命令**

| 原 v2.1 | v3.0 最終設計 | 變更原因 |
|---------|--------------|----------|
| `/atlas.changes` | `/atlas.history` | ✅ **重新命名**：更直覺的名稱 |
...
> **注意**：此版本已被 v3.0 取代，`/atlas.expert` 已移除
...
**目標版本變更**：
- v2.5 → **v2.6**（本提案設計用於 SourceAtlas v2.6）
...
1. **⚠️ 重要說明章節**（文檔開頭）
   - 說明目標版本變更（v2.5 → v2.6）
   - 命名變更對照表
   - v2.5 vs v2.6 的 Impact 分析區別

2. **與 v2.5 的區別說明**
   - `/atlas.impact`（v2.5）= 靜態分析（代碼結構）
   - `/atlas.coupling`（v2.6）= 時序分析（git 歷史）
   - 兩者互補使用的範例
